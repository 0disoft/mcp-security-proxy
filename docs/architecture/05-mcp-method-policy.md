...
When multiple descriptors use the same tool `name`, the proxy hides every descriptor with that name.
Duplicate names make the upstream tool identity ambiguous, and the call-time visible-tool state is
keyed by tool name.
