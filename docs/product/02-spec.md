...
- Hide every descriptor for duplicate visible tool names in a filtered discovery response so
  call-time state and client-visible discovery remain unambiguous.
...
- `mcp-security-proxy config-snippet`: emit a read-only `command` and `args` JSON descriptor for a
  validated policy, profile, and upstream stdio command. Host-specific targets emit official
  registration argv without executing the selected host or editing its configuration. Codex and
  project-scoped Gemini CLI targets are fixture-backed.
...
stderr, audit events go to the selected profile's file destination unless `--audit-log` overrides
